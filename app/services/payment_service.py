...
async def _resolve_payer_email(
    db: AsyncSession, installment: PremiumInstallment, actor: PlatformUser
) -> str:
    """Squad's VA initiate requires an email for payer notifications. Prefers
    the policyholder's on file; Policyholder.email is nullable (CLAUDE.md
    doesn't mandate it), so falls back to the initiating broker user's.
    """
    policy = await db.get(Policy, installment.policy_id)
    if policy is None:
        raise NotFoundError(f"policy {installment.policy_id} not found")
    policyholder = await db.get(Policyholder, policy.policyholder_id)
    if policyholder is not None and policyholder.email:
        return policyholder.email
    return actor.email


async def assert_installment_owned_by(
    db: AsyncSession,
    ...
    *,
    ...
    broker could pay off any other broker's/insurer's installment. Raises
    NotFoundError rather than a 403 so probing random ids can't distinguish
...
async def initiate_payment(
    ...
    *,
    ...
    """Mints a Squad Dynamic Virtual Account scoped to the exact installment
    amount and records a Payment row in 'initiated' status (docs/adr/0004).
    commission_config_id is resolved and locked here, before any Squad call,
    so a later rate change can never retroactively alter this row.
    """
    ...
    await assert_installment_owned_by(
        db, installment, broker_id=broker_id, insurance_company_id=insurance_company_id
    )
    ...
    email = await _resolve_payer_email(db, installment, actor)
    ...
    await record_audit_log(
        ...
        after_state={"amount_kobo": payment.amount_kobo, "squad_transaction_ref": transaction_ref},
    )
    await db.flush()
    return payment
...
async def resolve_payment_outcome(
    ...
    *,
    ...
    payment.failure_reason = f"unrecognized squad transaction_status: {status!r}"
    payment.raw_squad_response = requery_result.raw
    await record_audit_log(
        db,
        action="payment.failed",
