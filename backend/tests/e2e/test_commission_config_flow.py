...
async def test_list_insurance_company_brokers_scoped_to_active_assignments(
    client: AsyncClient, db_session: AsyncSession, fake_squad_client: FakeSquadClient
) -> None:
    admin_headers = await seed_and_activate_insureflow_admin(db_session, client)
    ...
    company, company_headers = await onboard_and_approve_insurance_company(
        client,
        admin_headers,
        name="Broker List Insurance",
        contact_email=f"admin-{uuid.uuid4()}@broker-list-insurance.example.com",
    )
    broker, _ = await onboard_and_approve_broker(
        client,
        admin_headers,
        name="Broker List Brokers",
        contact_email=f"admin-{uuid.uuid4()}@broker-list-brokers.example.com",
    )
    await assign_broker_to_insurer(
        client, admin_headers, broker_id=broker["id"], insurance_company_id=company["id"]
    )
    unassigned_broker, unassigned_headers = await onboard_and_approve_broker(
        client,
        admin_headers,
        name="Broker List Unassigned",
        contact_email=f"admin-{uuid.uuid4()}@broker-list-unassigned.example.com",
    )

    own_resp = await client.get(
        f"/api/v1/insurance-companies/{company['id']}/brokers", headers=company_headers
    )
    assert own_resp.status_code == 200, own_resp.text
    broker_ids = {b["id"] for b in own_resp.json()}
    assert broker_ids == {broker["id"]}
    assert unassigned_broker["id"] not in broker_ids

    admin_resp = await client.get(
        f"/api/v1/insurance-companies/{company['id']}/brokers", headers=admin_headers
    )
    assert admin_resp.status_code == 200, admin_resp.text
    assert {b["id"] for b in admin_resp.json()} == {broker["id"]}

    forbidden_resp = await client.get(
        f"/api/v1/insurance-companies/{company['id']}/brokers", headers=unassigned_headers
    )
