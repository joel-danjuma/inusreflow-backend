...
class PaymentBatch(UUIDPrimaryKeyMixin, TimestampMixin, Base):
    """One bulk-collection attempt covering many installments via the broker's
    permanent Squad Static Virtual Account for the combined total (PRD §8.3) --
    the broker is debited once and, on success, the insurer is settled once for
    ...
    failed attempt is never retried in place -- a fresh POST /payments/bulk
    creates a brand new PaymentBatch.
    """
    ...
    # Always null for SVA batches (no expiry); kept for schema compatibility.
    va_expires_at: Mapped[datetime | None] = mapped_column(DateTime(timezone=True), nullable=True)
    # Squad's own transaction_reference from the webhook; used for verify_transaction re-query.
