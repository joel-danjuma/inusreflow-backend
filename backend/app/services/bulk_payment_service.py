...
async def _load_payable_installments(
    ...
    *,
    broker_id: uuid.UUID,
) -> tuple[list[PremiumInstallment], uuid.UUID]:
    """Validates every installment up front -- exists, payable, owned by this
    broker -- before any Squad call, so one bad id fails the whole request
    rather than partially collecting (CLAUDE.md: "one debit, one
    ...
    load_owned_installment_policy -- a Locust run at the 500-item cap
    (tests/load/, docs/adr/0008-load-testing.md) measured the original
    per-id db.get() loop alone at ~8s, dominating request latency.

    A broker may now work with several insurers at once, so which insurer
    this batch belongs to is no longer caller-supplied -- it's derived from
    the installments' own policies, and a batch is rejected outright if it
    spans more than one insurer: bulk payments still settle as exactly one
    payout (CLAUDE.md), and there is no single insurer to settle a
    mixed-insurer batch to. A broker wanting to pay installments across two
    insurers submits two batches. Returns (installments, the one resolved
    ...
    resolved_insurance_company_id: uuid.UUID | None = None
    for installment_id in installment_ids:
        ...
        if resolved_insurance_company_id is None:
            resolved_insurance_company_id = policy.insurance_company_id
        elif policy.insurance_company_id != resolved_insurance_company_id:
            raise UnprocessableError(
                f"installment {installment_id} belongs to a different insurer than the "
                "rest of this batch -- bulk payments must be single-insurer"
            )
        installments.append(installment)
    # non-empty installment_ids guaranteed by the caller (initiate_bulk_payment)
    assert resolved_insurance_company_id is not None
    return installments, resolved_insurance_company_id
...
async def initiate_bulk_payment(
    ...
    *,
    ...
    """Records a PaymentBatch + one PaymentBatchItem per installment, all in
    'initiated' status (PRD §8.3), pointing to the broker's permanent Static
    Virtual Account for the combined total. The broker is debited once for the
    whole batch, never once per item. commission_config is resolved and locked
    here, before any financial state change, exactly as for a single payment.
    """
    ...
    installments, insurance_company_id = await _load_payable_installments(
        db, installment_ids, broker_id=broker_id
    )
