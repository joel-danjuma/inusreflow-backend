...
async def create_policyholder(
    ...
    *,
    ...
    """broker_id is the insurer-supplied broker this policyholder belongs to
    -- the caller (policyholders.py) has already validated it's one of this
    insurer's currently-active assignments before reaching here.
    """
    ...
    await record_audit_log(
        ...
        after_state={"full_name": full_name, "broker_id": str(broker_id)},
    )
    await db.flush()
    return policyholder
...
async def create_policy(
    ...
    *,
    ...
    coverage_amount_kobo: int | None = None,
    ...
    treating a mismatch as NotFound (rather than Forbidden) avoids confirming
    another broker's/insurer's policyholder ID exists. broker_id is the
    insurer-supplied broker this policy belongs to -- the caller
    (policies.py) has already validated it's one of this insurer's
    currently-active assignments before reaching here; the check below
    doubly confirms the named policyholder actually belongs to that same
    (broker, insurer) pair, not just one of the two independently.

    duration_months/coverage_*/broker_notes/internal_tags are free-form detail
    fields only -- none of them affect installment generation below.
    """
    policyholder = await db.get(Policyholder, policyholder_id)
    if (
        policyholder is None
        or policyholder.broker_id != broker_id
        or policyholder.insurance_company_id != insurance_company_id
    ):
        raise NotFoundError("policyholder not found")

    existing = await db.scalar(select(Policy).where(Policy.reference_number == reference_number))
    if existing is not None:
        raise ConflictError(f"a policy with reference number {reference_number!r} already exists")
    ...
    policy = Policy(
        ...
        coverage_amount_kobo=coverage_amount_kobo,
