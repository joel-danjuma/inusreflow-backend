...
import { onboardBroker } from "@/app/(auth)/onboard/broker/actions";

export function OnboardBrokerForm() {
  const [state, formAction, pending] = useActionState(onboardBroker, IDLE_STATE);
    ...
    return (
      <div className="space-y-3 rounded-base border border-border-success-subtle bg-success-soft px-4 py-3 text-sm text-fg-success-strong">
        <div>
          ...
          <p className="mt-1">
            {state.data.broker.name} is now <strong>pending</strong> Insureflow Admin approval.
          </p>
        </div>
        <div>
          ...
          <p className="mt-1 mb-2">
            ...
            immediately after. Full dashboard access still waits on approval and insurer
            assignment.
          </p>
  ...
  return (
    <form action={formAction} className="space-y-5">
      ...
      <Input id="name" name="name" label="Broker name" required error={state.fieldErrors?.name} />
      <Input
        id="contact_email"
        name="contact_email"
        label="Contact email"
